...
- 汇编代码。在CPU访存阶段，如果需要的数据不在cache中，那么CPU就需要从内存中调用数据。在这段时间里，CPU的算术执行模块会去执行其他的指令。为了提高实验的成功率，可以编写一段汇编码，让算术执行模块有事可干。代码引自[2]。在这段汇编代码中，`.rept 1000;`让整个程序段循环1000次，在循环内部，`add $0x141, %%eax;` 把0x141加到eax寄存器中，这段代码没有什么特殊的意义，只是为了让CPU的运算模块有事可干。
...
​	插入前文给出的内核模块，并把secret字符串写为"hello,I'm hacker.I hacked your computer by meltdown.Thank you."

​	按照FLUSH+RELOAD的操作思路，首先要将整个数组都放在内存中，并把cache中的数组清除。然后，在编写一个遍历函数，用来依次遍历整个数组，并测量访问时间，如果低于一个给定的阈值（该阈值通过反复实验得到），那么就增加它的可能性，多次尝试后取可能性最大的一组。在基本的一些功能实现后，再加入前文的异常处理程序和汇编代码部分，将他们整合在一起，就可以实现MeltdownAttack.c。
...
| 程序     |          |          |          |          |          |          |
| -------- | -------- | -------- | -------- | -------- | -------- | -------- |
| 运行时间 | 2.500543 | 2.457004 | 2.242928 | 2.468251 | 2.603869 | 2.512281 |
| 错码数   | 0        | 0        | 1        | 1        | 6        | 0        |

通过多次的尝试，发现了一些有趣的现象，若在短时间内连续攻击多次，攻击程序给出的错码率会急剧上升，而等过一段时间再次攻击后，就会发现错码数又降为0。此外，攻击程序泄漏出64B的数据平均需要2.4秒，这显然比较缓慢，需要进一步对程序进行优化。
